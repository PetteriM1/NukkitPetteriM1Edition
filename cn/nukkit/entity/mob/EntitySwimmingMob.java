...
public abstract class EntitySwimmingMob
...
implements EntityMob {
    ...
    public EntitySwimmingMob(FullChunk fullChunk, CompoundTag compoundTag) {
    ...
    public void setMaxDamage(int n, int n2) {
        block0: {
            if (n2 < 1 || n2 > 3) break block0;
            this.t[n2] = Math.max(n, this.getMinDamage(n2));
        }
    }
    ...
    public boolean onUpdate(int n) {
        block5: {
            ...
            if (!this.getServer().getMobAiEnabled() || this.isFriendly() && vector3 instanceof Player || !(vector3 instanceof Entity)) break block5;
            Entity entity = (Entity)vector3;
            if (!entity.closed && (vector3 != this.followTarget || this.u)) {
