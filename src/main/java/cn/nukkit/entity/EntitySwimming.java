...
public abstract class EntitySwimming extends BaseEntity {
    ...
    public Vector3 updateMove(int tickDiff) {
        ...
        if (!this.isImmobile()) {
            ...
            // Stay in water
            if (this.stayTime <= 0 && this.motionY >= 0 && (Math.abs(motionX) > 0 || Math.abs(motionZ) > 0) && !Block.isWater(this.level.getBlockIdAt(this.chunk, this.getFloorX(), NukkitMath.floorDouble(this.getY() + this.getHeight()), this.getFloorZ()))) {
                this.motionY = -0.05;
            }
