    ...
    public Vector3 updateMove(int tickDiff) {
        ...
        if (!this.isImmobile()) {
            ...
            if (this.followTarget != null && !this.followTarget.closed && this.followTarget.isAlive() && this.target != null && this.followTarget.canBeFollowed()) {
                ...
                double nDist;
                if (this.riding != null || diff <= 0.001 || !inWater && (this.stayTime > 0 || this.distanceSquared(this.followTarget) <= (nDist = ((this.getWidth() / 2 + 1) * nearbyDistanceMultiplier())) * nDist)) {
            ...
            if (this.target != null || !this.isLookupForTarget()) {
                ...
                double nDist;
                if (this.riding != null || diff <= 0.001 || !inWater && (this.stayTime > 0 || (distance = this.distanceSquared(this.target) <= (nDist = ((this.getWidth() / 2 + 1) * nearbyDistanceMultiplier())) * nDist))) {
                    if (!this.isInsideOfWater()) {
                        this.motionX = 0;
                        this.motionZ = 0;
                    }
