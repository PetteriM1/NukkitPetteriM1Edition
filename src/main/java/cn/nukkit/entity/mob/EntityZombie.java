...
public class EntityZombie extends EntityWalkingMob implements EntitySmite, EntityMobWithTool {
    ...
    public boolean entityBaseTick(int tickDiff) {
        ...
        if (!this.closed && this.isAlive()) {
            ...
            if (this.pickupItems && this.tool == null && this.age % 40 == 0 && this.isInTickingRange()) {
                ...
                for (Entity entity : entities) {
                    ...
                    if (item != null && item.getCount() == 1) {
                        ...
                        this.getViewers().forEach((id, pl) -> this.sendHandItems(pl));
                    }
                    break;
                }
            }
    ...
    private void saveTool() {
        if (this.tool != null) {
            ...
            this.namedTag.putBoolean("hasPlayerItem", this.hasPlayerItem);
        } else {
            this.namedTag.remove("Item");
            this.namedTag.remove("hasPlayerItem");
        }
    }
    ...
    private void setRandomTool() {
        if (Utils.rand(1, 10) == 5) {
            if (Utils.rand(1, 3) == 1) {
                this.tool = Item.get(Item.IRON_SWORD, Utils.rand(100, 246), 1);
            } else {
                this.tool = Item.get(Item.IRON_SHOVEL, Utils.rand(100, 246), 1);
            }
        }
    }
