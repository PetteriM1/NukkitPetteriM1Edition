...
public class EntityBee extends EntityFlyingMob implements EntityArthropod { // A mob because it needs to have an attack behavior
    ...
    public boolean onUpdate(int currentTick) {
        ...
        if (!isAlive() || !this.isInTickingRange()) {
            ...
            if (hasNectar() || getLevel().isRaining() || !getLevel().isAnimalSpawningAllowedByTime()) { // Search for hive
                ...
                if (!this.closed) { // No addOccupant
                    if (this.hive != null && this.distanceSquared(this.hive) > 25) {
                        BlockEntity hiveEntity = level.getBlockEntityIfLoaded(this.hive);
                        if (!(hiveEntity instanceof BlockEntityBeehive) || ((BlockEntityBeehive) hiveEntity).getOccupantsCount() < 3) {
                            target(this.hive);
                            return super.onUpdate(currentTick);
                        }
                    }
                        ...
                        for (int x = minX; x <= maxX; ++x) {
                            for (int y = minY; y <= maxY; ++y) {
                                ...
                                if ((block == BlockID.BEEHIVE || block == BlockID.BEE_NEST) && (newDist = temp.setComponents(x, y, z).distanceSquared(this)) < dist) {
                                    BlockEntity be;
                                    if ((be = level.getBlockEntityIfLoaded(temp)) instanceof BlockEntityBeehive) {
                                        if (((BlockEntityBeehive) be).getOccupantsCount() < 3) {
                                            dist = newDist;
                ...
                for (Entity entity : this.getLevel().getCollidingEntities(this.getBoundingBox().grow(10, 10, 10))) {
                    if (entity instanceof Player) {
                        Player p = (Player) entity;
                        if (!p.closed && p.isAlive()) {
                            int bId = p.getInventory().getItemInHandFast().getBlockId();
                            if (bId == BlockID.RED_FLOWER || bId == BlockID.DANDELION || bId == BlockID.WITHER_ROSE ||
                                    bId == BlockID.FLOWERING_AZALEA || bId == BlockID.AZALEA_LEAVES_FLOWERED ||
                                    bId == BlockID.CHORUS_FLOWER || bId == BlockID.MANGROVE_PROPAGULE || bId == BlockID.SPORE_BLOSSOM) {
                                this.followBlock = null;
                                this.target = entity;
                                this.stayTime = 0;
                                return super.onUpdate(currentTick);
                    ...
                    for (int z = minZ; z <= maxZ; ++z) {
                        for (int x = minX; x <= maxX; ++x) {
                            for (int y = minY; y <= maxY; ++y) {
                                ...
                                if (block == BlockID.RED_FLOWER || block == BlockID.DANDELION || block == BlockID.WITHER_ROSE || block == BlockID.FLOWERING_AZALEA || block == BlockID.AZALEA_LEAVES_FLOWERED || block == BlockID.CHORUS_FLOWER || block == BlockID.MANGROVE_PROPAGULE || block == BlockID.SPORE_BLOSSOM) {
                                    flowerCount++;
                                    if (Utils.rand(0, flowerCount - 1) == 0) {
                                        target(new Vector3(x + 0.5, y + 0.5, z + 0.5));
                                    }
                                }
                            }
    ...
    public boolean targetOption(EntityCreature creature, double distance) {
        if (creature instanceof Player && !isAngry() && !hasNectar && !level.isRaining() && level.isAnimalSpawningAllowedByTime()) {
            Player player = (Player) creature;
            if (!player.closed && player.isAlive()) {
                int blockId = player.getInventory().getItemInHandFast().getBlockId();
                if (blockId == BlockID.RED_FLOWER || blockId == BlockID.DANDELION || blockId == BlockID.WITHER_ROSE ||
                        blockId == BlockID.FLOWERING_AZALEA || blockId == BlockID.AZALEA_LEAVES_FLOWERED ||
                        blockId == BlockID.CHORUS_FLOWER || blockId == BlockID.MANGROVE_PROPAGULE || blockId == BlockID.SPORE_BLOSSOM) {
                    return distance <= 49;
                }
            }
        }
    ...
    public Vector3 updateMove(int tickDiff) {
        ...
        if (!this.isImmobile()) {
            ...
            if (this.followTarget != null && !this.followTarget.closed && this.followTarget.isAlive() && this.followTarget.canBeFollowed()) {
                ...
                double nDist;
                if (this.riding != null || diff == 0 || this.stayTime > 0 || this.distanceSquared(this.followTarget) <= (nDist = (this.getWidth() / 2 + 1)) * nDist) {
                    this.motionX = 0;
                    this.motionZ = 0;
                    if (this.riding == null) {
            ...
            if (this.followBlock != null) {
                ...
                double diff = Math.abs(x) + Math.abs(z);
                double nDist;
                if (this.riding != null || diff == 0 || this.stayTime > 0 || this.distanceSquared(this.followBlock) <= (nDist = (this.getWidth() / 2 + 1)) * nDist) {
                    this.motionX = 0;
                    this.motionZ = 0;
                    if (this.riding == null) {
                        this.motionY = this.getVerticalSpeed(0.01, y);
            ...
            if (this.target instanceof EntityCreature || !this.isLookupForTarget() || before != this.target) {
                ...
                double diff = Math.abs(x) + Math.abs(z);
                double nDist;
                if (this.riding != null || diff == 0 || this.stayTime > 0 || this.distanceSquared(this.target) <= (nDist = ((this.getWidth() / 2 + 1) * nearbyDistanceMultiplier())) * nDist) {
                    this.motionX = 0;
                    this.motionZ = 0;
                    if (this.riding == null) {
                        this.motionY = this.getVerticalSpeed(0.01, y);
            ...
            int groundBlock;
            if (this.stayTime <= 0 && this.motionY == 0 && (Math.abs(motionX) > 0 || Math.abs(motionZ) > 0) &&
                    (Block.isBlockSolidById((groundBlock = this.level.getBlockIdAt(this.chunk, this.getFloorX(), this.getFloorY() - 1, this.getFloorZ()))) || Block.isWater(groundBlock) || groundBlock == Block.LAVA || groundBlock == Block.STILL_LAVA)) {
                this.motionY = 0.05;
            }
