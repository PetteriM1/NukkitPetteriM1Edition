...
public class RepairItemTransaction extends InventoryTransaction {
    ...
    private boolean checkRecipeValid() {
        ...
        if (this.outputItem.getCount() != this.inputItem.getCount() && !this.isMapRecipe()) {
            if (Nukkit.DEBUG > 1) {
                source.getServer().getLogger().debug("failed: output count " + this.outputItem.getCount() + " != " + this.inputItem.getCount());
            }
            return false;
        }
