...
public enum EnumLevel {
    ...
    public static void initLevels() {
        ...
        if (server.getPropertyBoolean("allow-nether", true) && server.getPropertyBoolean("nether", true)) { // BC combat
        ...
        if (server.getPropertyBoolean("allow-the-end", true) && server.getPropertyBoolean("end", true)) { // BC combat
            ...
            String list = server.getPropertyString("multi-end-worlds", "");
            if (!list.trim().isEmpty()) {
                StringTokenizer tokenizer = new StringTokenizer(list, ", ");
                while (tokenizer.hasMoreTokens()) {
                    String world = tokenizer.nextToken();
                    Server.multiEndWorlds.add(world);
                    String end = world + "-the_end";
                    if (server.getLevelByName(end) == null) {
                        server.generateLevel(end, System.currentTimeMillis(), Generator.getGenerator(Generator.TYPE_THE_END));
                        server.loadLevel(end);
                    }
                }
            }
        }
