...
public class Level implements ChunkManager, Metadatable, GeneratorTaskFactory {
    ...
    public void addParticle(Particle particle, Player[] players, int count) {
        ...
        for (int protocolId : targets.keySet()) {
            ...
            if (packets != null) {
                ...
                broadcastPackets(protocolPlayers.toArray(new Player[0]), sendList.toArray(new DataPacket[0]));
            }
        }
    }
     ...
     */
    public void antiXrayOnBlockChange(@Nullable Player target, Vector3 block, int radius) {
        if (this.antiXrayEnabled()) {
            ...
            if (radius == 0) {
                ...
                server.getLogger().debug("antiXrayOnBlockChange unsupported radius " + radius, new Throwable(""));
                return;
            }
    ...
    private static void broadcastPackets(Player[] players, DataPacket[] packets) {
        for (DataPacket packet : packets) {
    ...
    public synchronized boolean setBlock(int x, int y, int z, BlockLayer layer, Block block, boolean direct, boolean update, boolean send) {
        ...
        if (update) {
            ...
            if (!ev.isCancelled()) {
                ...
                if (block instanceof BlockAir) this.antiXrayOnBlockChange(null, block, 1);
