...
public class ObjectCherryTree extends ObjectTree {
    ...
    private static void placeLeavesRow(
        ...
        final int extra = large ? 1 : 0;

        for (int dx = -range; dx <= range + extra; dx++) {
            for (int dz = -range; dz <= range + extra; dz++) {

                if (shouldSkipLocationSigned(random, dx, localY, dz, range, large)) {
                    continue;
                }
    ...
    private static boolean shouldSkipLocationSigned(
        ...
        int x = Math.min(
                Math.abs(localX),
                Math.abs(localX - (large ? 1 : 0))
        );

        int z = Math.min(
                Math.abs(localZ),
                Math.abs(localZ - (large ? 1 : 0))
        );

        return shouldSkipLocation(random, x, localY, z, range, large);
    }
