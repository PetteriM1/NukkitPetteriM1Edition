...
public class RakNetInterface implements RakNetServerListener, AdvancedSourceInterface {
    ...
    public boolean onConnectionRequest(InetSocketAddress address) {
        if (this.server.strongIPBans && this.server.getIPBans().isBanned(address.getHostString())) {
            this.server.getLogger().info(address.getHostString() + " connection rejected due to IP ban");
            return false;
        }
        return true;
    }
    ...
    public boolean process() {
        ...
        while ((session = this.sessionCreationQueue.poll()) != null) {
            ...
            try {
                ...
                this.server.getLogger().error("Failed to create Player", e);
