...
public class BiomeDefinitionListPacket extends DataPacket {
    ...
    private static BatchPacket CACHED_PACKET_OLD;
    ...
    public static BatchPacket getCachedPacket(int protocol) {
        if (protocol >= ProtocolInfo.v1_21_110) {
            ...
            if (CACHED_PACKET_554 == null) {
                ...
                try {
                    pk.tag = ByteStreams.toByteArray(Objects.requireNonNull(Nukkit.class.getClassLoader().getResourceAsStream("biome_definitions_554.dat")));
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
                pk.tryEncode();
                CACHED_PACKET_554 = pk.compress(Deflater.BEST_COMPRESSION);
            }
            ...
            if (CACHED_PACKET_486 == null) {
                ...
                try {
                    pk.tag = ByteStreams.toByteArray(Objects.requireNonNull(Nukkit.class.getClassLoader().getResourceAsStream("biome_definitions_486.dat")));
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
                pk.tryEncode();
            ...
            if (CACHED_PACKET_407 == null) {
                ...
                try {
                    pk.tag = ByteStreams.toByteArray(Objects.requireNonNull(Nukkit.class.getClassLoader().getResourceAsStream("biome_definitions_388.dat")));
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
                pk.tryEncode();
                CACHED_PACKET_407 = pk.compress(Deflater.BEST_COMPRESSION);
            }
            return CACHED_PACKET_407;
        } else {
            if (CACHED_PACKET_OLD == null) {
                BiomeDefinitionListPacket pk = new BiomeDefinitionListPacket();
                ...
                pk.tryEncode();
                CACHED_PACKET_OLD = pk.compress(Deflater.BEST_COMPRESSION);
            }
            return CACHED_PACKET_OLD;
        }
    }
