...
public class RakNetConstants {
    ...
    public static final short MINIMUM_MTU_SIZE = (short) 576;
    public static final short MAXIMUM_MTU_SIZE = (short) Math.min(Math.max(Server.getInstance().getPropertyInt("max-mtu", 1400), 576), 1492);
     ...
     */
    public static final long SESSION_TIMEOUT_NS = Server.getInstance().getPropertyInt("timeout-milliseconds", 20000) * 1_000_000L;

    /**
     * How many stale datagrams a {@link RakNetSession} can hold before being forcefully closed.
     */
    public static final int MAXIMUM_STALE_DATAGRAMS = 1500;

    /**
     * How many bytes per second can be received before client is disconnected for illegal packet
     */
    public static final int MAXIMUM_RECEIVED_BYTES = 10485760;
