...
public class RakNetServer extends RakNet {
    ...
    public void onOpenConnectionRequest1(ChannelHandlerContext ctx, DatagramPacket packet) {
        ...
        if (protocolVersion < 6 || protocolVersion > 12 || readableBytes < 400) {
            if (Nukkit.DEBUG > 1) {
                Server.getInstance().getLogger().debug(packet.sender() + " protocolVersion=" + protocolVersion + ", readableBytes=" + readableBytes);
            }
            return;
        }

        int mtu = readableBytes + 18 + (packet.sender().getAddress() instanceof Inet6Address ? 40 : 20) + UDP_HEADER_SIZE; // 1 (Packet ID), 16 (Magic), 1 (Protocol Version), 20/40 (IP Header)
