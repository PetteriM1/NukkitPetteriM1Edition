...
public class RakNetServer extends RakNet {
    ...
    private class ServerChannelInitializer extends ChannelInitializer<Channel> {
        ...
        protected void initChannel(Channel channel) throws Exception {
            ...
            if (!Server.getInstance().getPropertyBoolean("disable-rak-bans", false)) {
    ...
    public void onOpenConnectionRequest1(ChannelHandlerContext ctx, DatagramPacket packet) {
        ...
        if (session != null && session.getState() == RakNetState.CONNECTED) {
            Server.getInstance().getLogger().debug(packet.sender().getAddress().getHostAddress() + " already connected");
        } else if (Server.getInstance().strongIPBans && Server.getInstance().getIPBans().isBanned(packet.sender().getAddress().getHostAddress())) {
            Server.getInstance().getLogger().info(packet.sender().getAddress().getHostAddress() + " connection rejected due to IP ban");
        } else if (this.listener != null && !this.listener.onConnectionRequest(packet.sender(), packet.sender())) {
            Server.getInstance().getLogger().debug(packet.sender().getAddress().getHostAddress() + " connection banned");
            ...
            if (this.sessionsByAddress.putIfAbsent(packet.sender(), session) == null) {
                ...
                Server.getInstance().getLogger().debug("Previous session was still listed for " + packet.sender());
            }
        } else {
