...
public class RakNetServerSession extends RakNetSession {
    ...
    private void onOpenConnectionRequest2(ByteBuf buffer) {
        ...
        if (!RakNetUtils.verifyUnconnectedMagic(buffer)) {
            this.close(DisconnectReason.BAD_PACKET);
            return;
        }
    ...
    void sendOpenConnectionReply1() {
        if (++sentConnection1Replies > 20) {
            if (sentConnection1Replies == 21) {
    ...
    private void sendOpenConnectionReply2() {
        if (++sentConnection2Replies > 20) {
            if (sentConnection2Replies == 21) {
    ...
    private void sendConnectionRequestAccepted(long time) {
        if (++sentConnectionAcceptedReplies > 20) {
            if (sentConnectionAcceptedReplies == 21) {
        ...
        boolean ipv6 = this.address.getAddress() instanceof Inet6Address;
