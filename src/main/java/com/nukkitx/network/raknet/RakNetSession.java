...
public abstract class RakNetSession implements SessionConnection<ByteBuf> {
    ...
    private volatile long lastTouchedNano = System.nanoTime();
    ...
    public void onDatagram(ByteBuf buffer) {
        try {
            ...
            this.lastTouchedNano = System.nanoTime();
    ...
    protected void tick(long curTime) {
        if (System.nanoTime() - this.lastTouchedNano >= SESSION_TIMEOUT_NS) {
            this.close(DisconnectReason.TIMED_OUT);
            return;
        }

        if (this.lastRateLimitReset + 1000L < curTime) {
            this.receivedBytes = 0;
            this.lastRateLimitReset = curTime;
        }
