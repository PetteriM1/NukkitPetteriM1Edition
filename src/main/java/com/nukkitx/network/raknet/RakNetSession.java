...
public abstract class RakNetSession implements SessionConnection<ByteBuf> {
    ...
    volatile long lastTouched = RakNet.time();
    ...
    private volatile int splitQueuedBytes;
    private volatile int orderingQueuedBytes;
    ...
    final void initialize() {
        if (this.state != RakNetState.INITIALIZING) {
        ...
        this.orderReadIndex = 0;
        this.orderWriteIndex = 0;

        this.orderingHeaps = new FastBinaryMinHeap<>(64);
    ...
    private void deinitialize() {
        ...
        FastBinaryMinHeap<EncapsulatedPacket> orderingHeap = this.orderingHeaps;
        this.orderingHeaps = null;
        if (orderingHeap != null) {
    ...
    private EncapsulatedPacket getReassembledPacket(EncapsulatedPacket splitPacket) {
        ...
        if (this.splitQueuedBytes > RakNetConstants.MAXIMUM_RECEIVED_BYTES || this.orderingQueuedBytes > RakNetConstants.MAXIMUM_RECEIVED_BYTES) {
            Server.getInstance().getLogger().warning("MAXIMUM_RECEIVED_BYTES exceeded by " + this.address);
    ...
    public void onDatagram(ByteBuf buffer) {
        try {
            ...
            this.receivedBytes += buffer.readableBytes();
            if (this.receivedBytes > MAXIMUM_RECEIVED_BYTES) {
                Server.getInstance().getLogger().warning("MAXIMUM_RECEIVED_BYTES exceeded by " + this.address);
                this.disconnect(DisconnectReason.BAD_PACKET);
                return;
            }

            this.lastTouched = RakNet.time();
    ...
    private void onRakNetDatagram(ByteBuf buffer) {
        RakNetDatagram datagram = new RakNetDatagram(RakNet.time());
    ...
    private void onOrderedReceived(EncapsulatedPacket packet) {
        FastBinaryMinHeap<EncapsulatedPacket> binaryHeap = this.orderingHeaps;

        if (this.orderReadIndex < packet.orderingIndex) {
            ...
            return;
        } else if (this.orderReadIndex > packet.orderingIndex) {
        ...
        while ((queuedPacket = binaryHeap.peek()) != null) {
            if (queuedPacket.orderingIndex == this.orderReadIndex) {
                try {
    ...
    public final void tick() {
        if (this.eventLoop.inEventLoop()) {
            this.tick(RakNet.time());
        } else {
            this.eventLoop.execute(() -> this.tick(RakNet.time()));
        }
    }

    final void tick(long curTime) {
        this.lastTick = curTime;

        if (this.isClosed() || this.disconnected || this.state.ordinal() < RakNetState.INITIALIZED.ordinal()) {
            return;
        }
        ...
        if (!this.sendStaleDatagrams(curTime)) {
            log.info("Too many stale datagrams from " + this.address);
            ...
            if (range.end < range.start || range.end >= this.datagramWriteIndex) {
                if (Nukkit.DEBUG > 1) {
                    log.debug("Received {} with out-of-range indices [{}, {}] from {} (write index: {})",
                            nack ? "NACK" : "ACK", range.start, range.end, this.address, this.datagramWriteIndex);
                }
                continue;
            }
    ...
    private void onIncomingNack(RakNetDatagram datagram, long curTime) {
        ...
        this.sendDatagram(datagram, curTime, this.sentDatagrams);
    }
    ...
    private boolean sendStaleDatagrams(long curTime) {
        ...
        ConcurrentMap<Integer, RakNetDatagram> sent = new ConcurrentSkipListMap<>();
        ...
        while (iterator.hasNext()) {
            ...
            if (datagram.getNextSend() <= curTime) {
                ...
                iterator.remove();
                this.sendDatagram(datagram, curTime, sent);
            }
        }
    ...
    private void close0(DisconnectReason reason) {
        ...
        this.onClose();
        if (Nukkit.DEBUG > 2) {
            log.trace("RakNet Session {} closed: {}", this.address, reason);
        }
    ...
    public void send(ByteBuf buf, RakNetPriority priority, RakNetReliability reliability) {
        if (this.eventLoop.inEventLoop()) {
            ...
            this.eventLoop.execute(() -> this.send0(buf, priority, reliability));
        }
    }

    private void send0(ByteBuf buf, RakNetPriority priority, RakNetReliability reliability) {
        try {
            if (this.isClosed() || state.ordinal() < RakNetState.INITIALIZED.ordinal()) {
                // Session is not ready for RakNet datagrams.
                return;
            }
            EncapsulatedPacket[] packets = this.createEncapsulated(buf, reliability);
    ...
    private EncapsulatedPacket[] createEncapsulated(ByteBuf buffer, RakNetReliability reliability) {
        ...
        if (reliability.isOrdered()) {
            orderingIndex = this.orderWriteIndex++;
        }
        ...
        for (int i = 0, parts = buffers.length; i < parts; i++) {
            ...
            //packet.orderingChannel = (short) 0;
    ...
    private void sendDatagram(RakNetDatagram datagram, long time, ConcurrentMap<Integer, RakNetDatagram> sent) {
        ...
        try {
            ...
            for (EncapsulatedPacket packet : datagram.packets) {
                ...
                if (packet.reliability.isReliable()) {
                    ...
                    sent.put(datagram.sequenceIndex, datagram.retain()); // Keep for resending
                    break;
                }
            }
            ByteBuf buf = this.allocateBuffer(datagram.getSize());
            if (buf.writerIndex() >= this.adjustedMtu) {
                throw new IllegalArgumentException("Packet length was " + buf.writerIndex() + " but expected " + this.adjustedMtu);
            }
            datagram.encode(buf);
    ...
    private void onAcknowledge(ByteBuf buffer, Queue<IntRange> queue) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (start > end) {
                if (Nukkit.DEBUG > 1) {
                    Server.getInstance().getLogger().debug("ACK " + start + " > " + end + " from " + this.address);
                }
                this.disconnect(DisconnectReason.BAD_PACKET);
                return;
            }
