...
public class ServerDatagramHandler extends SimpleChannelInboundHandler<DatagramPacket> {
    ...
    private void onUnconnectedPing(ChannelHandlerContext ctx, DatagramPacket packet) {
        ...
        long diff = nowNs - lastPing[port];
        if (diff < 5_000_000) {
            if (Nukkit.DEBUG > 2) { // Don't have this spam with multiple server list entries added
        ...
        if (port < 1024) {
            if (Nukkit.DEBUG > 1) {
                Server.getInstance().getLogger().debug(packet.sender() + " ping port in reserved range");
            }
            return;
        }
