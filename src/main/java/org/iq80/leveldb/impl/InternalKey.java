...
public class InternalKey {
    ...
    public InternalKey(Slice userKey, long sequenceNumber, ValueType valueType) {
        if (userKey == null) throw new NullPointerException("userKey is null");
        if (sequenceNumber < 0) throw new IllegalArgumentException("sequenceNumber is negative");
    ...
    public InternalKey(Slice data) {
        ...
        if (data.length() < SIZE_OF_LONG) throw new IllegalArgumentException(String.format("data must be at least %s bytes", SIZE_OF_LONG));
