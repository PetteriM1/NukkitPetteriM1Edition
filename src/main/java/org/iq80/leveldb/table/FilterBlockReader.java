...
final class FilterBlockReader {
    ...
    public boolean keyMayMatch(long offset1, Slice key) {
        ...
        if (index < num) {
            final int start = contents.getInt(this.offset + (index << 2));
            final int limit = contents.getInt(this.offset + (index << 2) + 4);
