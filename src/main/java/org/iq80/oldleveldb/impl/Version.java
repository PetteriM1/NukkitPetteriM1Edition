...
import static org.iq80.oldleveldb.impl.VersionSet.MAX_GRAND_PARENT_OVERLAP_BYTES;
...
public class Version
    ...
    public Version(VersionSet versionSet) {
        ...
        Preconditions.checkArgument(NUM_LEVELS > 1);
    ...
    public LookupResult get(LookupKey key) {
        ...
        if (false) updateStats(readStats.getSeekFileLevel(), readStats.getSeekFile()); // We rather take slightly slower seeks than constantly attempting to compact
        return lookupResult;
    }
    ...
    public boolean overlapInLevel(int level, Slice smallestUserKey, Slice largestUserKey) {
        Preconditions.checkPositionIndex(level, levels.size());
        Preconditions.checkNotNull(smallestUserKey);
        Preconditions.checkNotNull(largestUserKey);
