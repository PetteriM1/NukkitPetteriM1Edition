...
import org.iq80.oldleveldb.util.InternalIterator;
import org.iq80.oldleveldb.util.Level0Iterator;
import org.iq80.oldleveldb.util.MergingIterator;
...
import static org.iq80.oldleveldb.impl.LogMonitors.throwExceptionMonitor;
...
public class VersionSet
    ...
    private static double maxBytesForLevel(int level) {
        ...
        double result = 1.048576E7;  // 10 * 1048576.0: Result for both level-0 and level-1
        while (level > 1) {
            result *= 10;
            level--;
        }
    ...
    public void recover()
        ...
        File currentFile = new File(databaseDir, Filename.currentFileName());
        Preconditions.checkState(currentFile.exists());
    ...
    public void setLastSequence(long newLastSequence) {
        Preconditions.checkArgument(newLastSequence >= lastSequence);
