...
public class Block
    ...
    public Block(Slice block, Comparator<Slice> comparator) {
        ...
        if (block.length() < SIZE_OF_INT) throw new IllegalArgumentException(String.format("Block is corrupt: size must be at least %s block", SIZE_OF_INT));
        ...
        if (restartCount > 0) {
            ...
            if (!(restartOffset < block.length() - SIZE_OF_INT)) throw new IllegalArgumentException("Block is corrupt: restart offset count is greater than block size");
