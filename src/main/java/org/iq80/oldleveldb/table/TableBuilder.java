...
import org.iq80.oldleveldb.util.Snappy;
import org.iq80.oldleveldb.util.Zlib;

import java.io.IOException;
import java.nio.ByteBuffer;
import java.nio.channels.FileChannel;

import static org.iq80.oldleveldb.impl.VersionSet.TARGET_FILE_SIZE;
...
public class TableBuilder {
    ...
    public TableBuilder(Options options, FileChannel fileChannel, UserComparator userComparator) {
        ...
        try {
            Preconditions.checkState(position == fileChannel.position());
        } catch (IOException e) {
    ...
    public void add(Slice key, Slice value)
        ...
        if (pendingIndexEntry) {
            Preconditions.checkState(dataBlockBuilder.isEmpty());
    ...
    private BlockHandle writeBlock(BlockBuilder blockBuilder)
        ...
        if (compressionType == CompressionType.ZLIB_RAW) {
            ...
            try {
                ...
                if (compressedSize < raw.length() - (raw.length() >> 3)) {
