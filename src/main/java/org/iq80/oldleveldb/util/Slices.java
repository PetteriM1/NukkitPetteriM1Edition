...
public final class Slices {
    ...
    public static Slice copiedBuffer(ByteBuffer source, int sourceOffset, int length) {
        ...
        return copiedBuffer((ByteBuffer) ((ByteBuffer) source).duplicate().order(ByteOrder.LITTLE_ENDIAN).clear().limit(newPosition + length).position(newPosition));
    }

    public static Slice copiedBuffer(ByteBuffer source) {
        if (source == null) throw new NullPointerException("source is null");
        Slice copy = allocate(source.limit() - source.position());
        copy.setBytes(0, ((ByteBuffer) source).duplicate().order(ByteOrder.LITTLE_ENDIAN));
        return copy;
    }
